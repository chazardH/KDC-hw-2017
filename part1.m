...
optimFunc = @(x)norm(target_modified-forwardKin(x,link_length))^2;
...
x = fmincon(optimFunc,x0,[],[],[],[],[min_roll; min_pitch; min_yaw], ...
    [max_roll; max_pitch; max_yaw],@(x)checkObstacles(x,obstacles,link_length),options);

optimFunc(x) %print the objective function value
