...
%
function [r, p, y] = cmaes( target, link_length, min_roll, max_roll, min_pitch, max_pitch, min_yaw, max_yaw, obstacles )
...
CostFunction=@(x)cmaObjectiveFunction(x,obstacles,link_length,target_modified);   % Cost Function

nVar=3*N;                % Number of Unknown (Decision) Variables

VarSize=[1 nVar];       % Decision Variables Matrix Size

VarMin=min(min([min_roll min_pitch min_yaw]));             % Lower Bound of Decision Variables
VarMax=max(max([max_roll max_pitch max_yaw]));            % Upper Bound of Decision Variables
